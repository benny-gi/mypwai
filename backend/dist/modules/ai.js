...
import { CreateCollectionCommand, DetectFacesCommand, DescribeCollectionCommand, DeleteFacesCommand, IndexFacesCommand, ListFacesCommand, RekognitionClient, ResourceNotFoundException, SearchFacesByImageCommand, } from '@aws-sdk/client-rekognition';
...
const describeCollectionSafe = async () => {
    ...
    if (!awsCollectionId)
        return { exists: false, faceCount: 0, message: 'AWS_REKOGNITION_COLLECTION_ID is not set' };
    try {
        const details = await rekognitionClient.send(new DescribeCollectionCommand({ CollectionId: awsCollectionId }));
        return {
            exists: true,
        ...
        if (error instanceof ResourceNotFoundException) {
            return { exists: false, faceCount: 0, message: 'Collection does not exist yet' };
        }
        throw error;
    }
};
...
const buildHallSummaries = (students, attendance, sessions, targetDate) => {
    return sessions
        .filter((session) => session.date === targetDate)
        .map((session) => {
        const sessionAttendance = attendance.filter((record) => record.date === targetDate &&
...
const buildRepeatedFailures = (events) => {
    const grouped = new Map();
    for (const event of events) {
        if (event.result !== 'failed' && event.result !== 'duplicate')
            continue;
...
const buildAnomalies = (students, attendance, sessions, scanEvents, targetDate) => {
    const anomalies = [];
    ...
    for (const session of sessions.filter((entry) => entry.date === targetDate)) {
        ...
        if (maxGap > 45 || lateFirstArrival) {
            anomalies.push(`${session.hall} shows irregular scan timing for ${session.courseCode}${lateFirstArrival ? ' with a delayed first check-in' : ''}.`);
        }
    }
    return anomalies;
};
const buildReport = (snapshot, question) => {
    ...
    const hallSummaries = buildHallSummaries(students, attendance, sessions, targetDate);
    const studentsPresentToday = new Set(attendance
    ...
    const totalPresent = uniqueCount(attendance
        .filter((record) => record.date === targetDate && record.status === 'Present')
        .map((record) => record.studentId));
...
const answerQuestion = (question, report) => {
...
router.post('/malpractice/frame', async (req, res) => {
    ...
    const hasExpectedStudent = normalizedStudentId.length > 0 &&
...
router.post('/malpractice/collection/enroll', async (req, res) => {
    ...
    try {
        ...
        try {
            const existingFaces = await rekognitionClient.send(new ListFacesCommand({ CollectionId: awsCollectionId, MaxResults: 1000 }));
...
router.get('/malpractice/collection/faces', async (_req, res) => {
    ...
    try {
        const response = await rekognitionClient.send(new ListFacesCommand({ CollectionId: awsCollectionId, MaxResults: 1000 }));
