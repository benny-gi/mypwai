...
const LoginPage = () => {
  return (
    <div className="relative min-h-screen w-full bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex flex-col text-white overflow-hidden">
      ...
      {/* Optional: Background Image Overlay (Uncomment to use an image instead) */}
      {/* <div className="absolute inset-0 bg-[url('https://source.unsplash.com/random/1920x1080/?technology')] bg-cover bg-center opacity-20 mix-blend-overlay"></div> */}

      {/* Header: Top Right Buttons */}
      <header className="absolute top-0 right-0 p-6 z-20 w-full flex justify-end items-center">
        <div className="flex gap-4">
          <button className="px-5 py-2.5 font-medium text-gray-300 hover:text-white transition-colors duration-200">
            Login
          </button>
          <button className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg shadow-lg shadow-blue-500/30 transition-all duration-200 transform hover:scale-105 active:scale-95">
            Sign Up
          </button>
        </div>
      </header>
